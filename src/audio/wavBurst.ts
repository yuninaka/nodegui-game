...
// Concatenates `repeatCount` copies of `clip`'s PCM data, each separated by `gapMs` of silence,
// into one buffer -- so a burst of clicks has sample-accurate timing instead of relying on the
// OS to schedule several separate plays on time.
export const buildBurstPcm = (clip: WavClip, repeatCount: number, gapMs: number): Buffer => {
...
export const wrapAsWavFile = (clip: Omit<WavClip, "pcmData">, pcmData: Buffer): Buffer => {
