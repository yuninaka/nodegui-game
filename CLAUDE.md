...
- さらに、手ごとに新しいプレイヤープロセスを起動する方式でも、出力デバイス（PulseAudioのシンク）が休止状態から復帰する遅延が毎回発生し、素早く連続で打つと次の音の鳴り始めが遅れる。`paplay --raw`/`aplay -t raw` をアプリ起動中ずっと1プロセスだけ常駐させ、そのstdinにPCMデータを書き込む方式にして解消（`src/audio/playClickSound.ts`）。`paplay` はファイル名引数（`-` を含む）を省略しないと標準入力を読まない点に注意（`aplay` は逆に `-` が必要）
